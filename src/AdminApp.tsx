...
function ProfileManagement({ownerId}:{ownerId:string}){
  ...
  async function inviteUser(event:FormEvent<HTMLFormElement>){
    ...
    formElement.reset();setNotice(`Поканата за ${email} е създадена. Копирайте линка от списъка по-долу.`);await refresh();
  }
  ...
  function activationLink(email:string){const url=new URL(`${baseUrl}trainings/admin.html`,window.location.origin);url.searchParams.set('activate','1');url.searchParams.set('email',email);return url.toString();}
  async function copyInviteLink(invite:UserInvite){
    try{await navigator.clipboard.writeText(activationLink(invite.email));setNotice(`Линкът за ${invite.email} е копиран.`);setError('');}
    catch{setError('Линкът не можа да се копира. Опитайте отново.');}
  }
  async function deleteInvite(invite:UserInvite){
    if(!supabase||!window.confirm(`Да изтрия ли поканата за ${invite.email}?`))return;
    setBusyId(invite.id);setError('');setNotice('');
    const {error:requestError}=await supabase.rpc('owner_delete_training_invite',{invite_id:invite.id});
    setBusyId('');
    if(requestError){setError(errorMessage(requestError));return;}
    setNotice(`Поканата за ${invite.email} е изтрита.`);await refresh();
  }

  async function deleteProfile(item:Profile){
    if(!supabase||item.id===ownerId||!window.confirm(`Да изтрия ли окончателно профила на ${item.email}? Потребителят повече няма да може да влиза.`))return;
    setBusyId(item.id);setError('');setNotice('');
    const {data:deleted,error:requestError}=await supabase.rpc('owner_delete_training_profile',{target_user_id:item.id});
    setBusyId('');
    if(requestError){setError(errorMessage(requestError));return;}
    if(!deleted){setError('Профилът не беше изтрит. Опитайте отново.');return;}
    setNotice(`Профилът на ${item.email} е изтрит.`);await refresh();
  }

  return <section className="users-panel owner-users-panel"><div className="admin-section-heading"><div><span>САМО ЗА OWNER</span><h2>Управление на профили</h2></div><strong>{profiles.length}</strong></div><p>Създайте покана, копирайте нейния линк и го изпратете лично на човека. Автоматичен имейл не се изпраща.</p><form className="invite-form" onSubmit={inviteUser}><label>ИМЕ<input name="name" placeholder="Име и фамилия"/></label><label>ИМЕЙЛ<input name="email" type="email" required placeholder="name@example.com"/></label><label>РОЛЯ<select name="role" defaultValue="editor"><option value="editor">Редактор</option><option value="admin">Администратор</option></select></label><button disabled={busyId==='invite'}>{busyId==='invite'?'Създаване…':'Създай покана'}</button></form>{error&&<div className="admin-alert error">{error}</div>}{notice&&<button className="admin-alert success" onClick={()=>setNotice('')}>{notice}<span>×</span></button>}{loading?<div className="admin-data-loading">Зареждане на профилите…</div>:<><div className="users-heading"><strong>Съществуващи профили</strong><span>{profiles.length}</span></div><div className="users-list owner-profile-list">{profiles.map(item=><div key={item.id}><span><strong>{item.display_name||item.email}</strong><small>{item.email}</small></span><select value={item.role} disabled={item.id===ownerId||busyId===item.id} onChange={event=>void updateAccess(item,event.target.value as ProfileRole,item.active)}><option value="owner" disabled>Owner</option><option value="admin">Администратор</option><option value="editor">Редактор</option></select>{item.id===ownerId?<b className="owner-self-badge">ВАШИЯТ ПРОФИЛ</b>:<div className="profile-actions"><button className={item.active?'deactivate':'activate'} disabled={busyId===item.id} onClick={()=>void updateAccess(item,item.role,!item.active)}>{busyId===item.id?'…':item.active?'Спри':'Активирай'}</button><button className="delete-profile" disabled={busyId===item.id} onClick={()=>void deleteProfile(item)}>Изтрий</button></div>}</div>)}</div>{invites.length>0&&<><div className="users-heading"><strong>Чакащи покани</strong><span>{invites.length}</span></div><div className="invite-list owner-invite-list">{invites.map(invite=><div key={invite.id}><span><strong>{invite.display_name||invite.email}</strong><small>{invite.email} · {invite.role==='admin'?'Администратор':'Редактор'}</small></span><div className="invite-actions"><button className="copy-invite" type="button" onClick={()=>void copyInviteLink(invite)}>Копирай линк</button><button className="delete-invite" type="button" disabled={busyId===invite.id} onClick={()=>void deleteInvite(invite)}>{busyId===invite.id?'…':'Изтрий'}</button></div></div>)}</div></>}</>}</section>;
}
...
function TariffBadge({tariff}:{tariff:TrainingRegistration['tariff']}){if(tariff==='none')return <span className="tariff-badge no-card">БЕЗ КАРТА</span>;if(tariff==='multisport')return <span className="tariff-badge multisport">MULTISPORT</span>;return <span className={`tariff-badge ${tariff}`}>{tariff==='card8'?'8':'12'}</span>;}
...
function LoginPanel(){
  const invitationParams=new URLSearchParams(window.location.search),invitedEmail=invitationParams.get('email')??'';
  const [mode,setMode]=useState<'login'|'activate'>(invitationParams.get('activate')==='1'?'activate':'login');const [emailValue,setEmailValue]=useState(invitedEmail);const [error,setError]=useState('');const [message,setMessage]=useState('');const [busy,setBusy]=useState(false);const [showPassword,setShowPassword]=useState(false);
...
function AdminLoading(){return <main className="login-shell"><div className="admin-loading"><strong className="fit-loading-mark" aria-label="FIT"><i>F</i><i>I</i><i>T</i></strong><span>Зареждане</span></div></main>;}
