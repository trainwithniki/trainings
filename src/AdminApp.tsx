...
function AdminDashboard(){
  ...
  return <main className="admin-shell live-admin">
    <header className="admin-header">
      <a className="admin-brand" href={`${baseUrl}trainings.html`}><span>FIT</span><strong>BODY CENTER</strong></a>
      <div className="admin-title"><span>УПРАВЛЕНИЕ</span><strong>Тренировки</strong></div>
    ...
    <section className="admin-only-heading"><div><span>АДМИН ПАНЕЛ</span><h1>Тренировки</h1></div><button className="admin-add-primary" onClick={()=>setEditor(null)}>＋ Нова тренировка</button></section>
    ...
    <section className="admin-live-section">
      ...
      {active.map(session=><ActiveTraining key={session.id} session={session} registrations={registrationsFor(session.id)} onEdit={()=>setEditor(session)} onStop={()=>changeStatus(session,'closed')} onDeleteRegistration={deleteRegistration}/>)}
    </section>
    ...
    <section className="admin-completed-section">
      ...
      <div className="completed-accordion">{completed.map(session=><CompletedTraining key={session.id} session={session} registrations={registrationsFor(session.id)} onEdit={()=>setEditor(session)} onDelete={()=>deleteSession(session)}/>)}</div>
    </section>
...
function ActiveTraining({session,registrations,onEdit,onStop,onDeleteRegistration}:{session:TrainingSession;registrations:TrainingRegistration[];onEdit:()=>void;onStop:()=>void;onDeleteRegistration:(item:TrainingRegistration)=>void}){
  ...
  return <article className="admin-training-card active-glow-card">
    ...
    {registrations.length===0?<p className="no-attendees">Все още няма записани.</p>:<div className="live-attendee-list">{registrations.map((person,index)=><AttendeeRow key={person.id} person={person} index={index} onDelete={()=>onDeleteRegistration(person)}/>)}</div>}
...
function AttendeeRow({person,index,onDelete}:{person:TrainingRegistration;index:number;onDelete?:()=>void}){
  return <div className="live-attendee-row"><span className="attendee-number">{index+1}</span><div className="attendee-identity"><strong>{person.name}</strong>{person.booked_by&&<small>Записан от: {person.booked_by}</small>}</div><TariffBadge tariff={person.tariff}/><a href={`tel:${person.phone}`}>{person.phone}</a>{onDelete&&<button className="attendee-delete" onClick={onDelete} aria-label={`Премахни ${person.name}`}>×</button>}</div>;
}
