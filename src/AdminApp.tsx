...
function AttendeeRow({person,index,onEdit,onDelete}:{person:TrainingRegistration;index:number;onEdit?:()=>void;onDelete?:()=>void}){
  ...
  return <div className="live-attendee-row"><span className="attendee-number">{index+1}.</span><div className="attendee-name-and-tariff"><div className="attendee-identity"><strong>{person.name}</strong>{person.booked_by&&<small>Записан от: {person.booked_by}</small>}</div><TariffBadge tariff={person.tariff}/></div><a className="attendee-phone" href={`tel:${person.phone}`}>{person.phone}</a><span className="attendee-booked-at" title={`Записан на ${created.date} в ${created.time} ч.`}><b>{created.shortDate}</b><small>{created.time}</small></span><div className="attendee-actions">{onEdit&&<button className="attendee-edit" onClick={onEdit} aria-label={`Редактирай ${person.name}`}>✎</button>}{onDelete&&<button className="attendee-delete" onClick={onDelete} aria-label={`Премахни ${person.name}`}>×</button>}</div></div>;
}
