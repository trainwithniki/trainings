...
export default function PublicApp(){
  ...
  return <main className="site-shell fbc-public live-public">
    ...
    <section className="section-block schedule-section exact-section"><h2 className="exact-section-title">Календар</h2><div className="calendar-card full-calendar live-calendar"><div className="exact-calendar-head"><button type="button" onClick={()=>setCalendarView(new Date(calendarView.getFullYear(),calendarView.getMonth()-1,1))} aria-label="Предишен месец">‹</button><strong>{months[calendarView.getMonth()]} {calendarView.getFullYear()}</strong><button type="button" onClick={()=>setCalendarView(new Date(calendarView.getFullYear(),calendarView.getMonth()+1,1))} aria-label="Следващ месец">›</button></div><div className="weekdays">{['ПН','ВТ','СР','ЧТ','ПТ','СБ','НД'].map(day=><span key={day}>{day}</span>)}</div><div className="calendar-grid">{calendarDays.map(item=>{const sessionState=item.session?stateOf(item.session):'';const classes=[item.muted?'muted':'',item.date===new Date().toISOString().slice(0,10)?'today':'',sessionState,item.session?.id===selectedId?'selected':''].filter(Boolean).join(' ');return <button type="button" key={item.date} className={classes} onClick={()=>{if(item.session){setSelectedId(item.session.id);setCalendarView(new Date(parseLocal(item.date).getFullYear(),parseLocal(item.date).getMonth(),1));}}} aria-label={item.session?`${item.session.title}, ${prettyDate(item.date)}`:prettyDate(item.date)}><span>{item.day}</span>{item.session&&sessionState==='open'&&<small>{Math.max(0,item.session.capacity-item.session.registration_count)} места</small>}</button>;})}</div><div className="calendar-legend"><span><i className="dot open-dot"/>Активна</span><span><i className="dot upcoming-dot"/>Предстояща</span><span><i className="dot completed-dot"/>Проведена</span></div></div></section>

    <section className="session-list-section exact-section"><h2 className="exact-section-title">Тренировки</h2><div className="session-group-title upcoming-title"><strong>Предстоящи</strong><span>{upcoming.length} тренировки</span></div>{upcoming.length?<div className="session-list">{upcoming.map(session=><SessionCard key={session.id} session={session} selected={session.id===selectedId} onSelect={()=>setSelectedId(session.id)}/>)}</div>:<div className="public-list-empty">Няма предстоящи тренировки.</div>}{completed.length>0&&<><div className="session-group-title completed-title"><strong>Проведени</strong><span>{completed.length}</span></div><div className="session-list">{completed.map(session=><SessionCard key={session.id} session={session} selected={session.id===selectedId} onSelect={()=>setSelectedId(session.id)}/>)}</div></>}</section>
...
function SessionCard({session,selected,onSelect}:{session:TrainingSession;selected:boolean;onSelect:()=>void}){
  const state=stateOf(session),date=parseLocal(session.date),free=Math.max(0,session.capacity-session.registration_count);
  return <article className={`exact-session-card ${state} ${selected?'selected':''}`}><button type="button" className="exact-session-top" onClick={onSelect}><div className="exact-session-date"><strong>{date.getDate()}.{String(date.getMonth()+1).padStart(2,'0')}</strong><span>{dayName(session.date)}</span><em>{shortTime(session.start_time)}</em></div><div className="exact-session-info"><span className={`exact-status ${state}`}>{state==='open'?'Записването е отворено':state==='completed'?'Проведена':'Предстояща'}</span><h3>{session.title}</h3><strong>{session.location.toUpperCase()}</strong></div></button><div className="exact-session-stats"><Stopwatch minutes={session.duration}/><div className="exact-capacity"><div><span>ЗАЕТИ<strong>{session.registration_count}</strong></span><span>СВОБОДНИ<strong>{free}</strong></span></div><i><b style={{width:`${Math.min(100,session.registration_count/session.capacity*100)}%`}}/></i></div></div>{state==='open'&&<button className="exact-session-book" type="button" onClick={onSelect}>ВИЖ ТРЕНИРОВКАТА</button>}</article>;
}
